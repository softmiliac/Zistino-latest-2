...
class BasketController extends GetxController {
  ...
  Future createOrder() async {
    ...
    OrderRqm orderRqm = OrderRqm(
        ...
        address2: '',
        createOrderDate:
            '${DateFormat('yyyy-MM-ddThh:mm:ss').format(DateTime.now())}.000Z',
        paymentTrackingCode: '',
        phone1: '',
        phone2: '',
        postStateNumber: '',
  ...
  increase(String? productID, bool fromCheckOutPage) {
    if (productID == null) {
      return;
    }
    try {
      ...
      if (item.isNotEmpty) {
        ...
        update();
        addToCart(items);
        if (fromCheckOutPage == false && Get.isSnackbarOpen == false) {
          showTheResult(
              resultType: SnackbarType.success,
              showTheResultType: ShowTheResultType.snackBar,
              hasBtn: true,
