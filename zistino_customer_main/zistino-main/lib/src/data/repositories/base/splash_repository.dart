...
import 'package:zistino/src/data/models/pro/category_model.dart';
...
import '../../providers/remote/base/config_api.dart';
...
class SplashRepositoryImpl extends SplashRepository {
  @override
  Future<bool> syncApp(ConfigRqm rqm) async {
    var a = getConfig(rqm);
  ...
  Future getConfig(ConfigRqm rqm) async {
    LocalStorageService pref = Get.find<LocalStorageService>();
    try {
      BaseResponse response = await ConfigApi().fetchConfig(rqm);

      if (response.data != null) {
        pref.setConfig(response.data);
      }else{
        pref.setConfig(ConfigModel().toJson());
      }

      return pref.getConfig;
    } catch (e) {
      AppLogger.e("$e");
      rethrow;
    }
