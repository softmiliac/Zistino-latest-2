...
import '../../models/sec/zone_model.dart';
...
class AddressRepositoryImpl extends AddressRepository {
  ...
  Future<List<AddressEntity>> fetchAll({bool fromLocal = false}) async {
    try {
      ...
      List<AddressEntity> result =
          AddressModel.fromJsonList(response.data as List);
      return result;
    } catch (e) {
      pref.setAddresses([]);
      rethrow;
  ...
  @override
  Future<BaseResponse> fetchAllZones(LazyRQM rqm,
      {bool fromLocal = true}) async {
    try {
      BaseResponse response = await AddressesAPI().searchBySp(rqm);
      if (response.data != null) {
        pref.setZones(response.data);
      } else {
        pref.setZones([]);
      }
      return response;
