...
class UserCompatibilitySerializer(serializers.ModelSerializer):
    ...
    isDriver = serializers.BooleanField(source='is_driver', read_only=True, help_text='Whether user is a driver (approved by manager)')
    ...
    class Meta:
        ...
        fields = [
            ...
            'phoneNumber', 'isDriver', 'imageUrl', 'thumbnail', 'companyName', 'companyNumber', 'code',
    ...
    def get_representative(self, obj):
        """Get representative code for user. This is a unique alphanumeric code generated automatically on user creation."""
        # Return the representative code (should be auto-generated by signal)
        if obj.representative and obj.representative.strip() and obj.representative.lower() != 'string':
            return obj.representative
        
        # If for some reason representative is empty, generate one
        # This should not happen if signal is working correctly
        try:
            import random
            import string
            ...
            from django.contrib.auth import get_user_model
            User = get_user_model()
            while User.objects.filter(representative=code).exists():
                code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
            
            # Save the generated code
            obj.representative = code
            obj.save(update_fields=['representative'])
            return code
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Error generating representative code for user {obj.id}: {str(e)}", exc_info=True)
            return None
